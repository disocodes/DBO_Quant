...
# MAGIC Run after `01_INGEST_DATA.py`. Enter a saved `portfolio_id` to use its latest holdings, or leave it blank and provide ad-hoc symbols/weights.
...
for n,d in [('catalog',current_catalog),('schema','openbb_quant'),('portfolio_id',''),('symbols','SPY,QQQ,IEF,GLD'),('weights_json','{"SPY":0.4,"QQQ":0.2,"IEF":0.25,"GLD":0.15}'),('initial_value','100000'),('horizon_days','1260'),('n_simulations','5000'),('method','historical_bootstrap'),('rebalance_every_days','21')]: dbutils.widgets.text(n,d)
CATALOG=dbutils.widgets.get('catalog'); SCHEMA=dbutils.widgets.get('schema'); PORTFOLIO_ID=dbutils.widgets.get('portfolio_id').strip()

if PORTFOLIO_ID:
    snapshots=spark.table(f'{CATALOG}.{SCHEMA}.portfolio_holdings').where(f"portfolio_id = '{PORTFOLIO_ID}'")
    if snapshots.count()==0: raise ValueError(f'No holdings found for portfolio_id={PORTFOLIO_ID}')
    latest=snapshots.agg(F.max('as_of_date').alias('d')).first()['d']
    holdings=snapshots.where(F.col('as_of_date')==latest).select('symbol','weight').toPandas()
    WEIGHTS=holdings.set_index('symbol')['weight'].astype(float)
    SYMBOLS=list(WEIGHTS.index)
    print('Using saved portfolio',PORTFOLIO_ID,'as of',latest)
else:
...
wide=prices.pivot_table(index='date',columns='symbol',values='price',aggfunc='last').sort_index().reindex(columns=SYMBOLS).ffill().dropna(how='any'); returns=wide.pct_change(fill_method=None).dropna(how='any')
